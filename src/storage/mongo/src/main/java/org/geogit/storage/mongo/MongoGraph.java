...
/**
 * MongoGraph encodes a general property graph into a MongoDB collection. The encoding works as
 * follows: * User-provided keys are simply ignored. Everything is based on mongo's autogenerated
 * '_id' identity. * For an edge, the endpoints are stored by object id (mongo's "$id") in "_in" and
 * "_out" fields. * The (optional) edge label is stored in "_label". * Any user-defined properties
 * are stored in a sub-document named "_properties". There are no restrictions on property names or
 * values beyond those imposed by Mongo. * For indexing, all index names are prefixed with 'e' for
 * edge indices or 'v' or vertex indices. Mongo's index system doesn't let us easily filter out
 * edges or vertices with the document conventions used here, so indexed queries always are modified
 * to require or disallow the presence of the '_in' and '_out' properties according to the index
 * type - the name just helps the system to know which one we expect.
 */
class MongoGraph implements Graph, KeyIndexableGraph {
    ...
    public Edge addEdge(Object id, Vertex out, Vertex in, String label) {
        ...
        DBObject edge = builder.append("_out", out.getId()).append("_in", in.getId())
                .append("_label", label).get();
    ...
    public Iterable<Edge> getEdges() {
        DBObject query = BasicDBObjectBuilder.start().push("_in").append("$exists", 1).pop()
                .push("_out").append("$exists", 1).pop().get();
        return getEdges(query);
    }

    public Iterable<Edge> getEdges(String key, Object value) {
        DBObject query = BasicDBObjectBuilder.start().push("_in").append("$exists", 1).pop()
                .push("_out").append("$exists", 1).pop().append("_properties." + key, value).get();
        return getEdges(query);
    }
    ...
    public Features getFeatures() {
        ...
        features.isPersistent = false; // TODO: Fix tests! they currently drop the database before
                                       // each run.
    ...
    public Iterable<Vertex> getVertices() {
        DBObject query = BasicDBObjectBuilder.start().push("_in").append("$exists", 0).pop()
                .push("_out").append("$exists", 0).pop().get();
        return getVertices(query);
    }

    public Iterable<Vertex> getVertices(String key, Object value) {
        DBObject query = BasicDBObjectBuilder.start().push("_in").append("$exists", 0).pop()
                .push("_out").append("$exists", 0).pop().append("_properties." + key, value).get();
        return getVertices(query);
    }
    ...
    public <T extends Element> void createKeyIndex(String key, Class<T> elementClass,
        ...
        if (elementClass.equals(Vertex.class)) {
            ...
            throw new UnsupportedOperationException(
                    "Can only index properties of edges or vertices, not " + elementClass);
        }
    }
    ...
    public <T extends Element> void dropKeyIndex(String key, Class<T> elementClass) {
        ...
        if (elementClass.equals(Vertex.class)) {
            ...
            throw new UnsupportedOperationException(
                    "Can only drop indices for edges or vertices, not " + elementClass);
        }
    ...
    private DBObject idQuery(Object id) {
        if (id instanceof String) {
            try {
                return new BasicDBObject("$or", new DBObject[] {
    ...
    private abstract class MElement implements Element {
        ...
        public Set<String> getPropertyKeys() {
            Object properties = record.get("_properties");
            if (properties instanceof DBObject) {
                return new HashSet<String>(((DBObject) properties).keySet());
            } else {
        ...
        public void setProperty(String key, Object value) {
            ...
            if (value instanceof Date) {
                throw new IllegalArgumentException(
                        "blocking date properties for blueprints test suite");
            }
    ...
    private class MVertex extends MElement implements Vertex {
        ...
        public Iterable<Edge> getEdges(Direction direction, String... labels) {
            ...
            switch (direction) {
            case BOTH:
                return Iterables.concat(getEdges(Direction.OUT, labels),
                        getEdges(Direction.IN, labels));
            case IN:
                path = new BasicDBObject("_in", getId());
                query = labels.length == 0 ? path : BasicDBObjectBuilder.start(path.toMap())
                        .push("_label").append("$in", labels).pop().get();
                return edges(collection.find(query));
            case OUT:
        ...
        public Iterable<Vertex> getVertices(Direction direction, String... labels) {
            ...
            case BOTH:
                return Iterables.concat(getVertices(Direction.OUT, labels),
        ...
        public String toString() {
            ...
            Object properties = record.get("_properties");
            if (properties instanceof DBObject) {
                for (Map.Entry<String, Object> entry : ((Map<String, Object>) ((DBObject) properties)
                        .toMap()).entrySet()) {
    ...
    private class MEdge extends MElement implements Edge {
        ...
        public String toString() {
            ...
            Object properties = record.get("_properties");
            if (properties instanceof DBObject) {
                for (Map.Entry<String, Object> entry : (Set<Map.Entry<String, Object>>) ((DBObject) properties)
                        .toMap().entrySet()) {
                    builder.append(entry.getKey());
                    builder.append("=");
                    builder.append(entry.getValue());
    ...
    private class MGraphQuery extends DefaultGraphQuery {
        ...
        protected BasicDBObjectBuilder mkMongoQuery(BasicDBObjectBuilder builder) {
            ...
            for (HasContainer container : hasContainers) {
                ...
                if (container.predicate instanceof com.tinkerpop.blueprints.Compare) {
                    ...
                    case EQUAL:
                        accum.add(new BasicDBObject(key, container.value));
                        break;
                    case GREATER_THAN:
                        accum.add(new BasicDBObject(key, new BasicDBObject("$gt", container.value)));
                        break;
                    case GREATER_THAN_EQUAL:
                        accum.add(new BasicDBObject(key, new BasicDBObject("$gte", container.value)));
                        break;
                    case LESS_THAN:
                        accum.add(new BasicDBObject(key, new BasicDBObject("$lt", container.value)));
                        break;
                    case LESS_THAN_EQUAL:
                        accum.add(new BasicDBObject(key, new BasicDBObject("$lte", container.value)));
                        break;
                    case NOT_EQUAL:
                        accum.add(new BasicDBObject(key, new BasicDBObject("$ne", container.value)));
                        break;
                    }
                    ...
                    case IN:
                        accum.add(new BasicDBObject(key, new BasicDBObject("$in", container.value)));
                        break;
                    case NOT_IN:
                        accum.add(new BasicDBObject(key, new BasicDBObject("$nin", container.value)));
                        break;
                    }
                }
            }
            if (accum.size() == 0) {
                ...
                for (Map.Entry<String, Object> e : ((Map<String, Object>) accum.get(0).toMap())
                        .entrySet()) {
        ...
        public Iterable<Edge> edges() {
            if (limit == 0) {
                ...
                BasicDBObjectBuilder builder = BasicDBObjectBuilder.start().push("_in")
                        .append("$exists", 1).pop().push("_out").append("$exists", 1).pop();
        ...
        public Iterable<Vertex> vertices() {
            if (limit == 0) {
                ...
                BasicDBObjectBuilder builder = BasicDBObjectBuilder.start().push("_in")
                        .append("$exists", 0).pop().push("_out").append("$exists", 0).pop();
    ...
    private class MVertexQuery extends DefaultVertexQuery {
        ...
        protected BasicDBObjectBuilder mkMongoQuery(BasicDBObjectBuilder builder) {
            ...
            if (labels != null && labels.length > 0) {
                accum.add(BasicDBObjectBuilder.start().push("_label").add("$in", labels).pop()
                        .get());
            }
            if (accum.size() == 0) {
                return builder;
        ...
        public Iterable<Edge> edges() {
            if (limit == 0) {
                ...
                case OUT:
                    ...
                    break;
                case BOTH:
                    edgeQuery = new BasicDBObject("$or", new DBObject[] {
                ...
                DBObject query = new BasicDBObject("$and", new DBObject[] { propertyQuery,
                        edgeQuery });
                return MongoGraph.this.getEdgesWithLimit(query, limit);
            }
        }
